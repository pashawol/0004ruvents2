...
const JSCCommon = {
  ...
  modalCall() {
    ...
    $('.sServises__btn').click(function () {
      $('#modal-servise .order').val(
        'Заявка на ' + $(this).parents('.sServises__wrap').find('.sServises__title--js').text(),
      );
...
function eventHandler() {
  ...
  $('.sClients__col ').each(function (i) {
    $(this).attr('data-wow-delay', delay * 0.01 * (i + 0.5) + 's');
  });

  $(' .sAbout__item').each(function (i) {
    $(this).attr('data-wow-delay', delay * 0.1 * (i + 0.5) + 's');
  });
